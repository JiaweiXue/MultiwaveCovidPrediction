...
def read_mobility_data(jcode23):
    all_mobility = dict()
    mobilityFilePath = "mobility_20210804"
...
def visual_loss(e_losses, vali_loss, test_loss):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('figure_supp/learning_curve.pdf',bbox_inches = 'tight')
    plt.show()
...
plt.legend()
plt.savefig('figure_supp/prediction_curve.pdf',bbox_inches = 'tight')
plt.show()
...
def getPredictionPlot(k):
    ...
    #plt.grid()  
    plt.savefig('peak4/' + str(k+1) + '.pdf',bbox_inches = 'tight')
    plt.show()
